...
namespace Prog_Rav_Ordini.Forms
{
    public partial class Principale : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void Inizializza_EConnetti_Database()
        {
            ...
            string errore = Utility_Generiche.Ricava_NomeServerENomeDatabase_Struttura3_DaFileCFG(ref Globale.Nome_Server, ref nome_Database);
            if (string.IsNullOrEmpty(errore))
            {
                Globale.ConnectionString = "XpoProvider=Firebird;DataSource=" + Globale.Nome_Server + ";User=SYSDBA;Password=birdfire;Database=Dati_Prog_Rav_Ordini;ServerType=0;Charset=ISO8859_1";
        ...
        private void Inserisci_Dentro_Inventari(string nome_file, string data)
        {
            ...
            progressBar1.Maximum = coll.Count();
            int prog = 0;
            foreach (INVENTARI item in coll)
            {
                // Cerco il giorno subito prima
                int cont = 0;
                while (cont < 20 && giorno_prima == null && string.Compare(data, prima_data) > 0)
                {
        ...
        //---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        private void simpleButton_ImportaUltimiDati_Click(object sender, EventArgs e)
        {
            Cursor.Current = Cursors.WaitCursor;
            av = new Avanzamento();
            av.Show();
